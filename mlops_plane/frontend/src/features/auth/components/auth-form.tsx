...
declare global {
  interface Window {
    __MLOPS_BOOTSTRAP__?: { username: string; password: string };
  }
}
...
const HIDDEN_LOGIN_USERNAME = "admin";
...
export function AuthForm({ mode }: AuthFormProps): ReactElement | null {
  ...
  useEffect(() => {
    function loadBootstrap() {
      const bootstrap = window.__MLOPS_BOOTSTRAP__;
  ...
  const currentPassword = password || window.__MLOPS_BOOTSTRAP__?.password || "";
  ...
  async function handleSubmit(event: SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (msg.includes("priority studio reset-password") && usePlatformStore.getState().deviceType === "windows") {
        msg = msg.replace(
          "priority studio reset-password",
          "citi-mlops reset-password",
        );
      }
      setError(msg);
    } finally {
