...
/** Number of priority results to pull up-front before yielding general results. */
const PRIORITY_PREFETCH = 20;
/** When the user searched for a specific publisher, show fewer priority results
 *  before the pinned (original publisher) model. */
const PRIORITY_PINNED_PREFETCH = 4;
...
/**
 * Creates a merged async generator that yields priority models first,
...
async function* mergedModelIterator(
  ...
  const priorityModelsIter = listModels({
    search: { query, owner: "", ...(task ? { task } : {}) },
    ...common,
  });
  ...
  // the Phase 1 priority iteration instead of blocking Phase 2.
  ...
  const limit = pinnedId ? PRIORITY_PINNED_PREFETCH : PRIORITY_PREFETCH;

  // Phase 1: pull & yield priority models first
  const seen = new Set<string>();
  let count = 0;
  for await (const model of priorityModelsIter) {
 ...
 * via modelInfo for full metadata), then the general priority listing.
 */
async function* priorityThenListingIterator(
  ...
  // Phase 2: yield general priority listing, skipping already-seen
  const generalIter = listModels({
    search: { owner: "", ...(task ? { task } : {}) },
    ...common,
  });
  for await (const model of generalIter) {
    const m = model as { name?: string };
...
export function useHfModelSearch(
  ...
  const { isPublisherQuery, searchQuery, pinnedId, trimmed } = useMemo(() => {
    ...
    const is = !!m && m[1].toLowerCase() !== "admin";
    return {
      isPublisherQuery: is,
  ...
  const createIter = useCallback(
    () => {
      if (!trimmed) {
        // No query: show priority models first (with full metadata), then general priority listing
      ...
      // slash-free, space-free segments), strip the org prefix so priority
      // variants surface, then pin the original publisher model after a small
      // batch of priority results.  Queries for priority models are left
  ...
  // Secondary sort guarantee: priority models always float to the top.
  // Skip when the user searched for a specific non-priority publisher
    ...
    () =>
      isPublisherQuery
        ...
        : [...search.results].sort((a, b) => {
            const aFirst = a.id.startsWith("") ? 0 : 1;
            const bFirst = b.id.startsWith("") ? 0 : 1;
